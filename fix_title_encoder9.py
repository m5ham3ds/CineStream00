...
old_block = """    // For general sites we use original Title so we don't break their search (they handle symbols themselves)
...
new_block = """    // We use the original encoded title for all sites as requested, so we don't break their search
    val searchUrl = when (currentSiteName) {
        "witanime.you" -> "https://witanime.you/?search_param=animes&s=$encodedPlusTitleOriginal"
        "w1.anime4up.rest" -> "https://w1.anime4up.rest/?s=$encodedTitleOriginal"
        "animeblkom.net" -> "https://animeblkom.net/search?query=$encodedPlusTitleOriginal"
...
if old_block in content:
    ...
    with open("app/src/main/java/com/example/ui/screens/player/ServerSelectionDialog.kt", "w") as f:
        f.write(content)
    print("Replaced URLs successfully!")
else:
