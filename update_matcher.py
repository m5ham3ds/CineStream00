...
old_block = """                // --- SMART SEARCH RESULT MATCHER ---
                if (serverItems.length === 0 && (!loc.includes('watch') && !loc.includes('episode') && !loc.includes('movie') && !loc.includes('play.php') && !loc.includes('video.php'))) {
...
new_block = """                // --- SMART SEARCH RESULT MATCHER ---
                if (serverItems.length === 0 && (!loc.includes('watch') && !loc.includes('episode') && !loc.includes('movie') && !loc.includes('play.php') && !loc.includes('video.php') && !loc.includes('/anime/') && !loc.includes('tvshow'))) {
                    ...
                    for(var k=0; k<allLinks.length; k++){
                        ...
                        if(h && h.startsWith('http') && !h.includes('login') && !h.includes('register') && !h.includes('?s=') && !h.includes('search')){
                    ...
                    if (results.length > 0) {
                        ...
                        if (bestMatchCount >= Math.max(1, words.length - 1)) {
                            targetResult = bestMatchElement;
                        }
                        
                        if (!targetResult && (loc.includes('?s=') || loc.includes('search') || loc.includes('query=') || loc.includes('keywords=') || loc.includes('?search_param='))) {
                            // try to fallback to old specific selectors
